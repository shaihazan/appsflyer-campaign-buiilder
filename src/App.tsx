...
export default function App() {
  ...
  const handleInputChange = (id: string, value: string) => {
    ...
    if (id === 'startDate') {
      ...
      if (value && !regex.test(value)) {
        setStartDateError('Format must be MM-YY (e.g. 04-26)');
      } else {
        setStartDateError('');
      }
    }
  ...
  const completedFieldsCount = useMemo(() => {
    return FIELDS.filter((field) => {
      if (field.id === 'freeText') return false; // Don't count optional field
  ...
  const isComplete = completedFieldsCount === FIELDS.length - 1; // 11 required fields
              ...
              <div className="flex flex-col gap-2 min-w-[200px]">
                <div className="flex justify-between items-center mb-1">
                  <span className="text-sm font-bold" style={{ color: BRAND_COLORS.dark }}>
                    {completedFieldsCount} / {FIELDS.length - 1} fields completed
                  </span>
                </div>
                <div className="w-full h-[6px] bg-[#e9f0f5] rounded-full overflow-hidden">
                  <motion.div
                    ...
                    initial={{ width: 0 }}
                    animate={{ width: `${(completedFieldsCount / (FIELDS.length - 1)) * 100}%` }}
          ...
          <div className="px-10 pb-10 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            {FIELDS.map((field) => (
              <div key={field.id} className="flex flex-col gap-2">
                <label className="text-[13px] font-semibold uppercase tracking-[0.05em] flex items-center gap-2" style={{ color: BRAND_COLORS.dark }}>
                  {field.label}
                  {field.id === 'freeText' && (
                    <span className="normal-case font-medium text-[11px] opacity-50">(Optional)</span>
                  )}
                </label>
                {field.type === 'dropdown' ? (
                  ...
                  <div className="relative">
                    <input
                      ...
                      value={formData[field.id] || ''}
                      onChange={(e) => handleInputChange(field.id, e.target.value)}
                      placeholder={field.id === 'startDate' ? 'MM-YY' : 'Enter text...'}
                      className={`w-full h-[48px] px-4 rounded-[10px] border-[1.5px] bg-white text-[15px] text-[#1a1a2e] transition-all outline-none hover:border-[#00C2FF] focus:border-[#00C2FF] focus:ring-[3px] focus:ring-[#00C2FF]/15 ${
                        (field.id === 'freeText' && freeTextError) || (field.id === 'startDate' && startDateError)
                          ? 'border-red-500 focus:ring-red-500' 
                          : (!formData[field.id] && field.id !== 'freeText' ? 'border-l-[3px] border-l-[#00C2FF] border-[#e2e8f0]' : 'border-[#e2e8f0]')
                      }`}
                    />
                    <AnimatePresence>
                      {field.id === 'freeText' && freeTextError && (
                        <motion.div
                      ...
                      {field.id === 'startDate' && startDateError && (
                        <motion.div
                          initial={{ opacity: 0, y: -5 }}
                          animate={{ opacity: 1, y: 0 }}
                          exit={{ opacity: 0, y: -5 }}
